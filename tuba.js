 ...
 *    
 *    Author: Stevan Guadalupe Plascencia-Gutierrez
 *    Date:   2.12.18
...
var fuelFieldset = document.getElementsByTagName("fieldset")[3];
    ...
    function verifyAcres(){
        ...
        try {
            if (!(acresBox.value > 0))
                throw "Please enter a number of acres greater tham 0"
        } catch (message) {
            validity = false;
            messageText = message;
            acresBox.value = "";
        }//Remove erroneous entry form input box
            finally{
...
function verifyCrops() {
    ...
    try { for (var i = 0; i < 7; i++) {
        ...
        //Catches the error message and displays it
        catch(message) { 
